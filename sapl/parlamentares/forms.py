...
class EditarNomePartidoForm(forms.Form):
    sigla = forms.CharField(label="Nova Sigla")
    ...
    def __init__(self, *args, **kwargs):
        ...
        row1 = to_row([('sigla', 2), ('nome', 8), ('data_alteracao', 2)])
        row2 = to_row([('partido_pk', 12)])

        partido = Partido.objects.get(id=kwargs['initial']['partido_pk'])

        self.helper = SaplFormHelper()
        self.helper.layout = SaplFormLayout(
            Fieldset(_('Alteração do nome do partido {}'.format(partido.nome)),
                    row1, row2))
